<template>
  <div class="container d-flex flex-column align-items-center">
    ...
    <div class=" d-flex justify-content-around align-items-center ">
      ...
      <input v-model="searchUsername"  type="search" class="px-2   border border-top form-control mr-2"  placeholder="Username">
      <input  v-model="searchEmail"  type="search" class="px-2   border border-top form-control"  placeholder="Email">
    </div>
  </div>
  ...
  <div class="w-100 overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow"  >
    <div data-v-9b3943d3="" class="flex items-center justify-around p-3">
          <div  class="col-4 d-flex justify-content-start">
            <button v-if="data.previous" @click="props.fetchUsers(data.previous)" class="border border-grey px-3  bigger-on-hover" title="Pagina precedente">
          ...
          <div  class="col-4 d-flex justify-content-center">
              <template v-for="(page, index) in getPaginationPages()" :key="index">
                ...
                <button v-else
                  @click="props.fetchUsers(get_url(page))" 
        ...
        <div  class="col-4 d-flex justify-content-end"> 
          <button   v-if="data.next" @click="props.fetchUsers(data.next)" class="border border-grey px-3  bigger-on-hover">
    ...
    <div class=" overflow-x-auto custom-scrollbar" >
      <table class=" table-fixed"  >
        <thead class="">
          <tr>
             <th  class=" px-5 py-3 text-left w-1/11 text-center sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400 text-center">ID</p>
            </th>
            <th  class=" px-2 py-3 text-left w-3/11 text-center sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Username</p>
            </th>
            <th class=" px-2 py-3 text-left text-center w-4/11 sm:px-6 overlow-auto">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Email</p>
            </th>
            <th  class=" px-2 py-3 text-left text-center w-3/11 sm:px-6">
              <p class="text-center  font-medium text-gray-500 text-theme-xs dark:text-gray-400">Azioni</p>
            </th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          <tr v-for="user in data.results" :key="user.id" class="border-t border-gray-100 dark:border-gray-800">
...
const animatingUserId = ref(null)
let timeout

current_url.value = props.apiUrl

watch([searchUsername, searchEmail], () => {
